...
class Emoji:
    def __init__(self,dict: dict,http: EndpointManager):
        ...
        self.user: Optional[User] = User(asyncio.run(http.send_request('get',f"/users/{dict.get("user")}")).data) if dict.get("user") is not None else None
